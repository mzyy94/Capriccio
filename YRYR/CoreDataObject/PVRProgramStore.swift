...
class PVRProgramStore: NSManagedObject {
	...
	func getOriginalObject() -> PVRProgram {
		return PVRProgram(id: id, title: title, fullTitle: fullTitle, subTitle: subTitle,
			...
			channel: channel.getOriginalObject(), episode: Int(episode), startTime: startTime,
			endTime: endTime, duration: NSTimeInterval(duration), userData: nil)
	}
	
	func setOriginalObject(object: PVRProgram) {
		...
		if let channel = PVRChannelStore.by("id", equalTo: object.channel.id).find().firstObject() as? PVRChannelStore {
			...
			channel.setOriginalObject(object.channel)
			channel.save()
			self.channel = channel
		}
	}
